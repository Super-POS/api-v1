...
export class AnalyticsService {
    ...
    async getDashboardSummary(start_date: string, end_date: string) {
        ...
        const [metrics, bestSellers, salesTrend, peakHours, wasteAnalysis] = await Promise.all([
            this.profitService.calculate(startDate, endDate),
            this.getBestSellingItems(start_date, end_date, 5),
            this.getSalesTrend(start_date, end_date, 'daily'),
            this.getPeakHours(start_date, end_date),
            this.getWasteAnalysis(start_date, end_date),
        ]);
    ...
    async getBestSellingItems(start_date: string, end_date: string, limit = 10) {
        ...
        const result = await OrderDetails.findAll({
            attributes: [
                'menu_id',
                [fn('SUM', col('order_details.qty')), 'total_qty'],
                [fn('SUM', literal('order_details.qty * order_details.unit_price')), 'total_revenue'],
            ],
            include: [
                {
                    model     : Order,
            ...
            group : ['order_details.menu_id', 'menu.id', 'menu.type.id'],
    ...
    async getProfitByProduct(start_date: string, end_date: string) {
        ...
        for (const m of menus) {
            if ((m as any).has_sizes) continue; // simplified: skip sized menus
            costMap.set(m.id, (m as any).cost ?? 0);
        }
        ...
        const sales = await OrderDetails.findAll({
            ...
            group: ['order_details.menu_id', 'menu.id'],
        } as any);
     ...
     *
     * Waste % = (Waste / Total Inventory) × 100
     */
    async getWasteAnalysis(start_date: string, end_date: string) {
        // Import wastage models dynamically to avoid circular deps at module level
        const IngredientWastage = (await import('@app/models/menu/wastage.model')).default;

        const wastages = await IngredientWastage.findAll({
            where: {
                created_at: { [Op.between]: [new Date(start_date), new Date(end_date + 'T23:59:59')] },
            },
            include: [{ model: MenuIngredient, attributes: ['id', 'name', 'unit', 'quantity'] }],
        });
